...
## Build 22 QA Pass (10 issues fixed — no new build number)
Walk memory integrity (evidence-enforced no-fabrication grounding driven by whether owner-scoped memory was actually retrieved; second-person "Last time, you…" summaries; owner-scoped isolation verified), "Last time" hardened to most-recent completed conversation, landing shows most-recent 1–2 active items per section (non-destructive, full history in Manage Walk Memory). Onboarding replay no longer crashes (routes to root tab then defers carousel emit). Walk closing auto-scrolls to "Take a breath" + auto-transitions on close-gate (no goodbye loop). Begin→Continue flicker fixed via skeleton. Walk input clears immediately on send + duplicate-submit guard. Scripture cross-chapter reference display "Genesis 1:26–31 & Genesis 2:1–3" (display-only). "Done for Today" persists per local calendar date with ✓ completed state, keeps Pray/Journal, resets next day, does not advance the plan. Tests: tests/test_build22_qa_fixes.py + tests/test_build22_walk_e2e.py; full walk suite green.
